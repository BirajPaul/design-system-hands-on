...
Button component provides interfaces for all types of the buttons. Different
types of the buttons are coded as different components and imported here.
...
<Preview>
	<Story
		...
		parameters={{
			...
			design: config({
				...
				url:
					'https://www.figma.com/file/FWi16PjbdbwImMWDnoFzrI/Brand-B?node-id=113%3A3473',
			}),
		}}
	...
	<Story
		name='Link'
		args={{ label: 'Button', isOutline: true, href: 'https://varya.me' }}>
		{(args) => <Button {...args} />}
	</Story>
	...
	<Story
		...
		args={{ label: 'Button', color: 'primary500' }}>
		{({ color, ...args }) => {
